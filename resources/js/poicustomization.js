...
document.addEventListener('DOMContentLoaded', () => {
  // Mapbox access token (public token provided by user)
  ...
  function fitToDrawings() {
    ...
    if (!bounds.isEmpty()) map.fitBounds(bounds, { padding: 30, maxZoom: 16, duration: 600 });
  }
  ...
  function boundsFromFeatureCollection(fc) {
    ...
    if (!fc || !fc.features) return b;

    const add = (c) => {
      const [lng, lat] = c;
      if (isValidLngLat(lng, lat)) b.extend([lng, lat]);
    };
  ...
  // Layers for points and line-position overlays
  function initProductLayers() {
    if (!map.getSource(PRODUCT_SOURCE_ID)) {
      map.addSource(PRODUCT_SOURCE_ID, { type: 'geojson', data: { type: 'FeatureCollection', features: [] } });
    }
    if (!map.getLayer(PRODUCT_POINT_LAYER_ID)) {
      map.addLayer({ id: PRODUCT_POINT_LAYER_ID, type: 'circle', source: PRODUCT_SOURCE_ID,
  ...
  // Robust parser for lineString which may be JSON or double-encoded string
  function parseLineStringToCoords(value) {
    let raw = value;

    // Try parse once if it's a string
    ...
    // If still a string (double-encoded), try again
    if (typeof raw === 'string') {
      try {
        raw = JSON.parse(raw);
      } catch (_) {
  ...
  function ensureLinePositionLayer() {
    if (!map.getSource(LINE_POS_SOURCE_ID)) {
      map.addSource(LINE_POS_SOURCE_ID, {
        type: 'geojson',
        data: {
  ...
  function clearProductLinesInDraw() {
    if (!productLineDrawIds || productLineDrawIds.length === 0) return;
    productLineDrawIds.forEach(id => { try { draw.delete(id); } catch (_) {} });
    productLineDrawIds = [];
  }
  ...
  async function openLinkModal(productId) {
    ...
    // Create or find an error message node
    ...
    try {
      ...
      return new Promise((resolve) => {
        ...
        const cleanup = () => {
          ...
          document.getElementById('link-save')
            .removeEventListener('click', onSave);
          document.removeEventListener('keydown', onKey);
        };

        document.getElementById('link-cancel').addEventListener('click', onCancel);
        document.getElementById('link-save').addEventListener('click', onSave);
        document.addEventListener('keydown', onKey);
      });
    } catch (err) {
      // Keep modal open, show error and allow user to cancel
  ...
  // Click handler for overlay circles: link unlinked temp lines
  map.on('click', LINE_POS_CIRCLE_LAYER_ID, async (e) => {
    const f = e.features && e.features[0];
    if (!f) return;

    if (f.properties && String(f.properties.linked) === 'true') return;

    const current = productsById.get(String(document.getElementById('product-select').value));
    if (!current) return;

    const drawData = draw.getAll();
    if (!drawData || !drawData.features) return;

    // Find nearest temp line by midpoint
    let targetLine = null;
    let minDist = Infinity;

    drawData.features.forEach((g) => {
      if (!g.geometry || g.geometry.type !== 'LineString') return;
    ...
    try {
      ...
      // Remove the temp line from Draw and our tracking list
      draw.delete(targetLine.id);
      productLineDrawIds = productLineDrawIds.filter((id) => id !== targetLine.id);

      // Reload products and re-render current
  ...
  function renderProduct(product) {
    ...
    if (existingLinesFc.features.length > 0) {
      ...
      productLineDrawIds.push(...ids);
    }
    // Update points
    const src = map.getSource(PRODUCT_SOURCE_ID); if (src) src.setData(pointsFc);
    // Refresh overlay (midpoints) with temp state
    const tempData = draw.getAll();
    const tempLinesFc = { type: 'FeatureCollection', features: (tempData.features || []).filter(f => f.geometry && f.geometry.type === 'LineString') };
    refreshLinePositionOverlay(product, tempLinesFc);

    // Centering/zoom
    const lng = Number(product.defaultLongitude), lat = Number(product.defaultLatitude);
    const hasCenter = isValidLngLat(lng, lat); const zoomConfigured = parseZoom(product.zoom);
    const lngDelta = Number(product.defaultLongitudeDelta), latDelta = Number(product.defaultLatitudeDelta);
    const hasDeltas = Number.isFinite(lngDelta) && Number.isFinite(latDelta) && lngDelta > 0 && latDelta > 0;
    if (hasCenter && hasDeltas) {
      const sw = [lng - lngDelta/2, lat - latDelta/2], ne = [lng + lngDelta/2, lat + latDelta/2];
      const b = new mapboxgl.LngLatBounds(sw, ne); map.fitBounds(b, { padding: 30, maxZoom: 18, duration: 600 });
    } else if (hasCenter && zoomConfigured !== null) {
      map.flyTo({ center: [lng, lat], zoom: zoomConfigured, essential: true });
    } else {
      const allFc = { type: 'FeatureCollection', features: [...existingLinesFc.features, ...pointsFc.features] };
      const b = boundsFromFeatureCollection(allFc);
      if (!b.isEmpty()) map.fitBounds(b, { padding: 30, maxZoom: 17, duration: 600 });
      else if (hasCenter) map.flyTo({ center: [lng, lat], zoom: 15, essential: true });
    }
  }
  ...
  async function loadProducts() {
    ...
    try {
      const res = await fetch('/api/products'); const json = await res.json();
      const products = (json && (json.products || json.audios || [])) || [];
      selectEl.innerHTML = '';
      products.forEach(p => { productsById.set(String(p.id), p); const opt = document.createElement('option'); opt.value = String(p.id); opt.textContent = p.name || `Product ${p.id}`; selectEl.appendChild(opt); });
      const storedId = localStorage.getItem('poicustomization.selectedProductId');
      let selected = storedId ? products.find(p => String(p.id) === storedId) : null;
      if (!selected && products.length > 0) selected = products[0];
      if (selected) { selectEl.value = String(selected.id); renderProduct(selected); }
      else { const opt = document.createElement('option'); opt.value=''; opt.textContent='No products found'; opt.disabled=true; opt.selected=true; selectEl.appendChild(opt); }
    } catch (_) {
      selectEl.innerHTML = ''; const opt = document.createElement('option'); opt.value=''; opt.textContent='Failed to load products'; opt.disabled=true; opt.selected=true; selectEl.appendChild(opt);
    }
  }

  document.getElementById('copy-btn').addEventListener('click', async () => {
    try { await navigator.clipboard.writeText(outputEl.value); const btn = document.getElementById('copy-btn'); const prev = btn.textContent; btn.textContent = 'Copied!'; setTimeout(() => (btn.textContent = prev), 1000); }
    catch { alert('Copy failed. You can manually select and copy the text.'); }
  });
  document.getElementById('clear-btn').addEventListener('click', () => {
    draw.deleteAll(); productLineDrawIds = []; updateOutput();
    const current = productsById.get(String(document.getElementById('product-select').value)); if (current) refreshLinePositionOverlay(current, { type: 'FeatureCollection', features: [] });
  });
  document.getElementById('fit-btn').addEventListener('click', fitToDrawings);
  document.getElementById('product-select').addEventListener('change', (e) => {
    const id = e.target.value; localStorage.setItem('poicustomization.selectedProductId', id); renderProduct(productsById.get(id));
  });

  map.on('load', () => { updateOutput(); initProductLayers(); ensureLinePositionLayer(); loadProducts(); });
  map.on('draw.create', () => { updateOutput(); const current = productsById.get(String(document.getElementById('product-select').value)); if (!current) return; const tempData = draw.getAll(); const tempLinesFc = { type: 'FeatureCollection', features: (tempData.features || []).filter(f => f.geometry && f.geometry.type === 'LineString') }; refreshLinePositionOverlay(current, tempLinesFc); });
  map.on('draw.delete', () => { updateOutput(); const current = productsById.get(String(document.getElementById('product-select').value)); if (!current) return; const tempData = draw.getAll(); const tempLinesFc = { type: 'FeatureCollection', features: (tempData.features || []).filter(f => f.geometry && f.geometry.type === 'LineString') }; refreshLinePositionOverlay(current, tempLinesFc); });
  map.on('draw.update', () => { updateOutput(); const current = productsById.get(String(document.getElementById('product-select').value)); if (!current) return; const tempData = draw.getAll(); const tempLinesFc = { type: 'FeatureCollection', features: (tempData.features || []).filter(f => f.geometry && f.geometry.type === 'LineString') }; refreshLinePositionOverlay(current, tempLinesFc); });
});
